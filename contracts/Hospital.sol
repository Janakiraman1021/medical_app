...
contract Hospital {
    ...
    function changeStaff(address _a) external onlyOwner {
        staff = _a;
    }

    function getPatientDetails() external view returns (patient memory) { //only Patient
    ...
    function deleteAppointments(uint256 _no) external onlyStaff {
        require(_no <= ap_No, "Enter a valid Appointment ID");
        uint256 pos = 0;

        for (uint256 i = 0; i < ap_No; i++) {
